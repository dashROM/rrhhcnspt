...
class ModeloPersonaContratos {
	...
	static public function mdlMostrarPersonaContratos($tabla, $item, $valor1, $valor2) {
		...
		if ($item != null) {
			...
			if ($valor2 != null) {
				...
				$sql = "SELECT pc.id_persona_contrato, pc.cod_contrato, l.id_lugar, l.codificacion, l.nombre_lugar, e.id_establecimiento, e.nombre_establecimiento, concat_ws(' ', pe.paterno_persona, pe.materno_persona, pe.nombre_persona) AS nombre_completo, concat_ws(' ', pe.ci_persona, pe.ext_ci_persona) AS ci_persona, pe.fecha_nacimiento, co.id_contrato, co.nombre_contrato, ca.id_cargo, ca.nombre_cargo, pc.tipo_contratacion, ca.observacion_cargo, pc.inicio_contrato, pc.dias_contrato, pc.fin_contrato, pc.estado_contrato, pc.observaciones_contrato, pe.id_persona, s.id_suplencia, s.tipo_suplencia, pc.archivo_contrato, pc.documento_ampliacion, pc.ampliacion, pc.resolucion_ministerial, pc.id_memorandum, m.nro_memorandum, m.fecha_memorandum FROM personas pe, persona_contratos pc, cargos ca, contratos co, establecimientos e, suplencias s, lugares l, memorandums m WHERE pe.id_persona = pc.id_persona AND ca.id_cargo = pc.id_cargo AND co.id_contrato = pc.id_contrato AND e.id_establecimiento = pc.id_establecimiento AND s.id_suplencia = pc.id_suplencia AND l.id_lugar = pc.id_lugar AND m.id_memorandum = pc.id_memorandum AND pc.$item = :$item";
			...
			$sql = "SELECT pc.id_persona_contrato, pc.cod_contrato, l.id_lugar, l.codificacion, l.nombre_lugar, e.id_establecimiento, e.nombre_establecimiento, co.id_contrato, co.nombre_contrato, ca.id_cargo, ca.nombre_cargo, ca.observacion_cargo, pc.inicio_contrato, pc.dias_contrato, pc.fin_contrato, pc.estado_contrato, pc.observaciones_contrato, pe.id_persona, pc.archivo_contrato, pc.ampliacion, pc.tipo_contratacion FROM personas pe, persona_contratos pc, cargos ca, contratos co, establecimientos e, lugares l WHERE pe.id_persona = pc.id_persona AND ca.id_cargo = pc.id_cargo AND co.id_contrato = pc.id_contrato AND e.id_establecimiento = pc.id_establecimiento AND l.id_lugar = pc.id_lugar AND pc.id_persona = :id_persona ORDER BY pc.id_persona_contrato DESC";
	...
	static public function mdlNuevoPersonaContrato($tabla, $datos){
		...
		$stmt = Conexion::conectarPG()->prepare("INSERT INTO $tabla(id_lugar, id_establecimiento, id_persona, id_cargo, inicio_contrato, dias_contrato, fin_contrato, id_contrato, id_suplencia, id_memorandum, estado_contrato, observaciones_contrato, documento_contrato, nro_cod_contrato, cod_contrato, tipo_contratacion, ampliacion, recurrencia) VALUES (:id_lugar, :id_establecimiento, :id_persona, :id_cargo, :inicio_contrato, :dias_contrato, :fin_contrato, :id_contrato, :id_suplencia, :id_memorandum, :estado_contrato, :observaciones_contrato, :documento_contrato, :nro_cod_contrato, :cod_contrato, :tipo_contratacion, :ampliacion, :recurrencia)");
